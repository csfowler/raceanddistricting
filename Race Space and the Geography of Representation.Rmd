...
totSteps = 10 #number of steps in the gerry chain. 10 is ideal for testing 10k-30k for full eval

#py_discover_config() #identify the available conda environments
use_condaenv(condaenv='gerry', required = TRUE) #this is set up based on the instructions for installing gerryChain using conda. See online docs for gerryChain
...
updaters = {
    "population": updaters.Tally("TOTALPOP", alias="population"),
    "cut_edges": cut_edges,
    "bvap": updaters.Tally("POP18BLACK",alias="bvap"),
    "vap": updaters.Tally("POP18PLUS",alias="vap")
}
...
for v in graph.nodes:
    pop=pop+graph.nodes()[v]["TOTALPOP"]

new_plan = recursive_tree_part(graph=graph, parts=range(num_dist), pop_target=pop/num_dist,pop_col="TOTALPOP",epsilon= .01,node_repeats=1)
...
proposal = partial(recom, pop_col = "TOTALPOP", pop_target = pop/num_dist, epsilon = 0.05, node_repeats = 3)
...
ggdensity(data =distribs_18, x = "DistrictSquaredError",
            ...
            subtitle = paste0("For ",numPlans," randomly generated plans containing at least two districts with >37% BVAP"))+
  ...
  xlab("Difference from commonwealth percentage")+
